...
We ran **real local Laya inference on all 1,328 issues**, including 168 open and 1,160 closed issues. Pull requests were excluded.
...
Median model time was **7.548 seconds per issue** for all 25 label decisions together.
...
- Actual inference device: **mps**, **FP32** (mixed precision: False). CPU thread limit: 2.
- Platform: `macOS-26.6.2-arm64-arm-64bit`.
- Median / p95 / p99: **7.548s / 8.370s / 8.870s** per issue.
- Mean / maximum: **7.374s / 12.308s**.
- Total measured inference: **163.2 minutes**.
- Amortized throughput: **8.1 issues/minute**.
- Inference shards: **1**. Load/checksum figures below are summed across shards, not parallel wall time.
- Recorded model load: **4.80s**; download/cache lookup plus checksum: **0.71s**.
...
A GitHub-hosted CPU runner is a different machine and has different latency. This report measures the local run. See README.md for separate hosted execution evidence.
...
- Run fingerprint: `1eb71c0c218e55634425422d2da72b5fe26537cf01fd9f59c3adc46b9e4d6a99`.
- Run completed: `2026-09-25T11:32:36.145077+00:00`.
...
- Raw per-issue probabilities and timing: [runs/baseline/predictions.jsonl](runs/baseline/predictions.jsonl).
- Machine-readable metrics and all proposals: [runs/baseline/metrics.json](runs/baseline/metrics.json) and [runs/baseline/proposals.csv](runs/baseline/proposals.csv).
